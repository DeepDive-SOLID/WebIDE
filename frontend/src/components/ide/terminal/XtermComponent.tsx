...
import { setInput, setOutput } from "../../../stores/terminalSlice";
import { useDispatch, useSelector } from "react-redux";
import type { RootState } from "../../../stores";
import axios from "axios";
import type { XtermProps } from "../../../types/ide";

const XtermComponent = ({ isInputDisabled, setIsInputDisabled, codeId, height }: XtermProps) => {
  ...
  const test = useCallback(
    async (codeToProcess: string) => {
      ...
      try {
        const backendUrl = "http://localhost:8080/docker/custom";
        const response = await axios.post(backendUrl, { codeFileId: codeId, questionId: 1, input: codeToProcess });
        ...
        dispatch(setOutput(response.data.toString() || "백엔드로부터 출력이 없습니다."));
      } catch (error) {
        console.error("백엔드 코드 처리 중 오류 발생:", error);
        // 에러 메시지를 터미널에 출력 (빨간색으로)
        dispatch(setOutput(`\x1b[31m오류: ${error instanceof Error ? error.message : String(error)}\x1b[0m`));
      }
    },
    [dispatch]
  ); // dispatch는 안정적이므로 의존성 배열에 포함
  ...
  }, [setInput, dispatch, test, restart]);
  ...
  useEffect(() => {
    if (output && xtermRef.current) {
      ...
      dispatch(setOutput(null)); // 출력 후 상태 초기화
      dispatch(setInput(""));
    }
  }, [output, dispatch]);

  return <div ref={terminalRef} style={{ width: "100%", height: height }} />;
};
