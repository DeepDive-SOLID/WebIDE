...
const AddFileModal = ({
  ...
  useEffect(() => {
    ...
    const problems = boxList
      .filter((item) => item.isProblem)
      .map(({ directoryId, title }) => ({ directoryId, title }));

    dispatch(setProblemEntries(problems));
  }, [boxList, dispatch]);
  ...
  const onSubmit = async (data: FormValues) => {
    const validChecked = data.testcases.filter(
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <div className={styles.inputGroup}>
          ...
          <div className={styles.formControl}>
            ...
            />
            {errors.problem && (
              <p className={styles.warning}>⚠ 문제를 작성해주세요.</p>
            )}
          </div>
          ...
          <div className={styles.formControl}>
            ...
            <textarea
              id="inputDesc"
              {...register("inputDesc", { required: true })}
              placeholder="입력 설명"
            />
            {errors.inputDesc && (
              <p className={styles.warning}>⚠ 입력 설명이 필요합니다.</p>
            )}
          </div>
          ...
          <div className={styles.formControl}>
            ...
            <textarea
              id="outputDesc"
              {...register("outputDesc", { required: true })}
              placeholder="출력 설명"
            />
            {errors.outputDesc && (
              <p className={styles.warning}>⚠ 출력 설명이 필요합니다.</p>
            )}
          </div>
          ...
          <div className={styles.labeledInput}>
            ...
            {errors.timeLimit && (
              <p className={styles.warning}>⚠ 제한 시간을 입력해주세요.</p>
            )}
          </div>
          ...
          <div className={styles.labeledInput}>
            ...
            {errors.memoryLimit && (
              <p className={styles.warning}>⚠ 메모리 제한을 입력해주세요.</p>
            )}
          </div>
        </div>
        ...
        <div className={styles.testcaseSection}>
          ...
          {fields.map((field, index) => (
            <div key={field.id} className={styles.testcaseRow}>
              <input
                type="checkbox"
                {...register(`testcases.${index}.checked` as const)}
              />
              <input
          ...
          {watchTestcases.filter(
            (tc) => tc.checked && tc.input.trim() && tc.output.trim()
          ).length < 3 && (
            <p className={styles.testcaseWarning}>
              ⚠ 체크된 테스트 케이스가 3개 이상 필요합니다.
            </p>
          )}
          ...
          <button
            ...
            onClick={() => append({ input: "", output: "", checked: false })}
