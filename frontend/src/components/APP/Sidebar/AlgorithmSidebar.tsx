...
const AlgorithmSidebar = ({ containerId }: AlgorithmSidebarProps) => {
  ...
  useEffect(() => {
    ...
    dispatch(setDirectoryId(Number(id)));
        ...
        if (questionProgressData && Array.isArray(questionProgressData)) {
          ...
          questionList.forEach((question) => {
            if (question.directoryId) {
              ...
              const boxItem = boxList.find((item) => item.directoryId === question.directoryId && item.hasQuestion === true);
              if (boxItem) {
            ...
            <ContextMenu
              ...
              onCreate={async (type) => {
                ...
                if (type === "folder") {
                  ...
                  const siblingItems = boxList.filter((item) => item.parentId === (parent?.id ?? null));

                  const hasDuplicateProblem = siblingItems.some((item) => item.title === title && item.isProblem);
        ...
        <div className={styles.teamStatus}>
          ...
          {(() => {
            ...
            memberGroups.forEach((memberDataList, memberId) => {
              ...
              const progress = selectedId ? selectedLangData?.averageProgress || 0 : selectedLangData?.averageProgress || 0;
              const currentLanguage = selectedLangData?.language || "N/A";

              // 완료한 언어들을 표시할 문자열 생성
              // 디렉터리(문제)를 선택했을 때만 언어 표시
              const languageDisplay = selectedId ? (completedLanguages.length > 0 ? completedLanguages.join(", ") : currentLanguage) : "N/A";

              console.log(`Team member progress - Name: ${memberName}, Progress: ${progress}%, Completed: ${completedLanguages.join(", ")}`);

              allBars.push(<Bargraph key={`${memberId}-${selectedLanguage}`} name={memberName} language={languageDisplay} success={progress} total={100} />);
            });

            // 빈 슬롯 채우기
            const emptySlots = Math.max(0, 5 - allBars.length);
            for (let i = 0; i < emptySlots; i++) {
              allBars.push(<Bargraph key={`empty-${i}`} name='' language='' success={0} total={100} />);
            }
